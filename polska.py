...
color_map = ['red','green','blue','orange','purple','yellow','cyan','magenta']
def draw_circle(x, y, label, color=None):
    x, y = coords(x, y)
    turtle.penup()
    turtle.goto(x, y-20)
    turtle.pendown()
    if color is not None:
        ...
        turtle.circle(20)
    turtle.write(label, font=("Verdana", 18, "bold"))
    turtle.penup()
...
def draw_map(edges=graf, col=None):
    ...
    if col is None:
        ...
        for k, c in col:
            x,y,_ = slownik[k]
            draw_circle(x, y, str(c), color_map[c])
